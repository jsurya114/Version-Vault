...
async function main() {
  ...
  // Create a bad ts file and package.json with typescript
  await container
  ...
  await container
    .exec({
      Cmd: [
        '/bin/sh',
        '-c',
        'echo "{ \\"devDependencies\\": { \\"typescript\\": \\"^4.9.5\\" } }" > package.json',
      ],
    })
    .then((e) => e.start());
  ...
  await new Promise((r) =>
    execInstall.start({ Detach: false, Tty: true }).then((s) => s.on('end', r)),
  );
