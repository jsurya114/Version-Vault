...
const features = [
  { icon: '🗄️', title: 'Repositories', desc: 'Full access and control over all your private and public repos.' },
  { icon: '✍️', title: 'Commits', desc: 'Blazing fast history tracking with rich metadata for every change.' },
  { icon: '🔀', title: 'Pull Requests', desc: 'Streamlined PR workflows with comments, reviews, and CI gates.' },
  { icon: '🐛', title: 'Issues', desc: 'Integrated issue management for your entire development team.' },
];
    ...
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex items-center gap-6">
            {['Product', 'Pricing', 'Docs'].map((item) => (
              <span key={item} className="text-gray-400 hover:text-white text-sm cursor-pointer transition">{item}</span>
            ))}
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Link to={ROUTES.LOGIN} className="text-gray-300 hover:text-white text-sm transition px-3 py-1.5">
            Sign in
          </Link>
          <Link to={ROUTES.REGISTER} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-1.5 rounded-lg transition">
            Sign up
          </Link>
        </div>
      </nav>
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-10 text-center">
        ...
        <p className="text-gray-400 text-base max-w-xl mx-auto mb-8 leading-relaxed">
          The next-generation code hosting platform built for speed, security, and seamless collaboration. Deploy faster than ever.
        </p>

        <div className="flex items-center justify-center gap-3 mb-14">
          <Link to={ROUTES.REGISTER} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded-lg transition text-sm">
            Start Getting More
          </Link>
          <Link to={ROUTES.LOGIN} className="bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white font-semibold px-6 py-2.5 rounded-lg transition text-sm">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden text-left shadow-2xl max-w-2xl mx-auto">
          ...
          <div className="p-5 font-mono text-sm space-y-1.5">
            <p><span className="text-gray-500">$</span> <span className="text-green-400">git init version-vault</span></p>
            ...
            <p><span className="text-yellow-400">warning</span> <span className="text-gray-400">Checking connectivity... done</span></p>
            <p><span className="text-blue-400">commit</span> <span className="text-gray-400">3f9a2c1</span> <span className="text-gray-500">— feat: add dark mode support</span></p>
            <p><span className="text-green-400">success</span> <span className="text-gray-400">Repository initialized successfully ✓</span></p>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          <div className="flex -space-x-2">
            {['bg-blue-500', 'bg-purple-500', 'bg-green-500', 'bg-orange-500', 'bg-pink-500'].map((color, i) => (
              <div key={i} className={`w-8 h-8 rounded-full ${color} border-2 border-gray-950 flex items-center justify-center text-xs font-bold`}>
      ...
      <section className="border-t border-gray-800 py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {features.map((f) => (
              <div key={f.title} className="bg-gray-900 border border-gray-800 hover:border-gray-700 rounded-xl p-5 transition">
        ...
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-12 items-center">
          <div className="md:w-1/2">
            <h2 className="text-3xl font-bold text-white mb-4">
              World-class security is<br />baked in by default.
            </h2>
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              Protect your code at every step. VersionVault automatically scans for errors and alerts you to vulnerabilities in your dependencies before they become problems.
            </p>
            <div className="space-y-3">
              <div>
                <p className="text-white text-sm font-medium">Secret Scanning</p>
                <p className="text-gray-500 text-xs">Automatically detects API keys & tokens across all protocols.</p>
              </div>
              <div>
                <p className="text-white text-sm font-medium">Dependency Graph</p>
                <p className="text-gray-500 text-xs">Understand your dependency tree and security vulnerabilities.</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-gray-900 border border-yellow-500/30 rounded-xl p-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between bg-gray-800 rounded-lg px-3 py-2">
                  ...
                  <span className="bg-red-500/20 text-red-400 text-xs px-2 py-0.5 rounded">critical</span>
                </div>
                <div className="flex items-center justify-between bg-gray-800 rounded-lg px-3 py-2">
                  <span className="text-gray-300 text-xs">lodash version vulnerable</span>
                  <span className="bg-yellow-500/20 text-yellow-400 text-xs px-2 py-0.5 rounded">fix it</span>
                </div>
              </div>
            </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto bg-blue-600 rounded-2xl px-10 py-14 text-center">
          ...
          <p className="text-blue-100 text-sm mb-8 max-w-md mx-auto">
            Join over 90 million developers who are already using VersionVault to build amazing things every single day.
          </p>
          <div className="flex items-center justify-center gap-4">
            <Link to={ROUTES.REGISTER} className="bg-white hover:bg-gray-100 text-blue-600 font-semibold px-6 py-2.5 rounded-lg transition text-sm">
              Create Free Account
            </Link>
            <Link to={ROUTES.LOGIN} className="border border-white/40 hover:border-white text-white font-semibold px-6 py-2.5 rounded-lg transition text-sm">
              Talk to Sales
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-8">
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-gray-600 text-xs cursor-pointer hover:text-gray-400">Public Group</span>
          ...
          {[
            ...
            <div key={col.title}>
              ...
              <div className="flex flex-col gap-2">
                {col.links.map((link) => (
                  <span key={link} className="text-gray-500 text-xs cursor-pointer hover:text-gray-300 transition">{link}</span>
                ))}
              </div>
            </div>
          ))}
          <div>
            ...
            <Link to={ROUTES.ADMIN_LOGIN} className="text-gray-500 text-xs hover:text-gray-300 transition">
              Admin Portal
            </Link>
          </div>
        </div>
