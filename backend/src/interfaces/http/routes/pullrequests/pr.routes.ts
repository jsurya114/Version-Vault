...
router.patch('/:username/:reponame/:id/request-merge', authMiddleware, writeAccessMiddleware, (req, res, next) =>
  prController.requestMerge(req, res, next),
);

// Owner approves and merges
router.patch('/:username/:reponame/:id/approve-merge', authMiddleware, ownerMiddleware, (req, res, next) =>
  prController.approveMerge(req, res, next),
);

// Owner rejects merge request
router.patch('/:username/:reponame/:id/reject-merge', authMiddleware, ownerMiddleware, (req, res, next) =>
  prController.rejectMerge(req, res, next),
);
