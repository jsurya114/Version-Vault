...
const FollowItem = React.memo(
  ({ username, onClick }: { username: string; onClick: (u: string) => void }) => (
    <div
    ...
    >
      ...
      <div className="flex-1">
        <p className="text-white font-semibold text-sm group-hover:text-blue-400">{username}</p>
...
export const FollowListModal: React.FC<FollowListModalProps> = React.memo(
            ...
            {data.length === 0 ? (
              ...
              <div className="space-y-1">
                {data.map((item) => {
                  ...
                  return <FollowItem key={item.id} username={username} onClick={handleUserClick} />;
                })}
              </div>
            )}
          </div>
