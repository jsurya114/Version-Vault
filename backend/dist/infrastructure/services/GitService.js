...
let GitService = class GitService {
    static { GitService_1 = this; }
    repoBasePath;
    constructor() {
        this.repoBasePath = path.resolve(env_config_1.envConfig.GIT_REPO_PATH || './repos');
        //create base directory if it doesnt exist
        if (!fs.existsSync(this.repoBasePath)) {
            fs.mkdirSync(this.repoBasePath, { recursive: true });
        }
    }
    getRepoPath(ownerUsername, repoName) {
    ...
    buildConflictContent(ourContent, theirContent, targetBranch, sourceBranch) {
        ...
        for (let i = 0; i < maxLen; i++) {
            ...
            if (oursLine === theirsLine) {
                ...
                inConflict = true;
                if (oursLine !== undefined)
                    oursBlock.push(oursLine);
                if (theirsLine !== undefined)
                    theirsBlock.push(theirsLine);
            }
        }
    ...
    async createBranch(ownerUsername, repoName, newBranch, fromBranch) {
        ...
        try {
            ...
            if (msg.includes('not a valid object name')) {
                throw new ConflictError_1.ConflictError(`Cannot create a branch from '${fromBranch}'. The repository might be empty. Please commit at least one file first.`);
            }
            throw new Error(`Failed to create branch: ${msg}`, { cause: error });
        }
    }
    async initBareRepo(ownerUsername, repoName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        ...
        //init bare repo
        await (0, simple_git_1.default)(repoPath).init(['--bare']);
    }
    async deleteBareRepo(ownerUsername, repoName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        if (fs.existsSync(repoPath)) {
            fs.rmSync(repoPath, { recursive: true, force: true });
        }
    }
    repoExits(ownerUsername, repoName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        return fs.existsSync(repoPath);
    }
    async ensureRepoExists(ownerUsername, repoName) {
        if (!this.repoExits(ownerUsername, repoName)) {
            await this.initBareRepo(ownerUsername, repoName);
        }
    }
    getFullRepoPath(ownerUsername, repoName) {
        return this.getRepoPath(ownerUsername, repoName);
    }
    /**
     * Calculate the actual disk storage of a bare repository in bytes.
     */
    getRepoStorageBytes(ownerUsername, repoName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        if (!fs.existsSync(repoPath))
            return 0;
     ...
     */
    async getBranchCount(ownerUsername, repoName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
    ...
    async getLanguageStats(ownerUsername, repoName, branch = 'main') {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
            ...
            for (const line of result.trim().split('\n')) {
                ...
                if (baseName === 'dockerfile' || baseName.startsWith('dockerfile.')) {
                    ...
                    lang = GitService_1.EXT_LANGUAGE_MAP[ext];
                }
                if (!lang)
                    continue; // Skip unknown / config files
    ...
    async getFiles(ownerUsername, repoName, branch = 'main', filePath = '', recursive = false) {
        ...
        try {
            ...
            Logger_1.logger.info(`[getFiles] Repo: ${repoPath}, Branch: ${branch}, treePath: ${treePath}`);
            const namesResult = await git.raw(namesArgs);
            const typesResult = await git.raw(typesArgs);
            Logger_1.logger.info(`[getFiles] namesResult length: ${namesResult ? namesResult.length : 0}`);
            if (!namesResult)
                return [];
            ...
            console.error(`[getFiles] ERROR for ${ownerUsername}/${repoName} branch=${branch} path=${filePath}`, error);
            return [];
        }
    }
    async getFileContent(ownerUsername, repoName, filePath, branch = 'main') {
        await this.ensureRepoExists(ownerUsername, repoName);
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
    ...
    async getCommits(ownerUsername, repoName, branch = 'main', limit = 20) {
        await this.ensureRepoExists(ownerUsername, repoName);
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
    ...
    async getBranches(ownerUsername, repoName) {
        await this.ensureRepoExists(ownerUsername, repoName);
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
            const defaultBranch = 'main';
            const format = '%(refname:short)|%(committerdate:iso8601)|%(authorname)|%(authoremail)|%(subject)';
            const result = await git.raw(['branch', `--format=${format}`]);
            const branches = await Promise.all(result
                .trim()
                .split('\n')
                .filter(Boolean)
                .map(async (l) => {
    ...
    async mergeBranch(ownerUsername, repoName, sourceBranch, targetBranch) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
    ...
    async deleteBranch(ownerUsername, repoName, branchName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
    ...
    async commitChanges(ownerUsername, repoName, branch, message, filePath, content, authorName, authorEmail) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        ...
        try {
            ...
            const gitWithIndex = (0, simple_git_1.default)(repoPath).env({ ...process.env, GIT_INDEX_FILE: indexFile });
            ...
            const commitHash = (await git
                ...
                .raw(['commit-tree', treeHash, '-p', parentHash, '-m', message])).trim();
    ...
    async compareGitCommits(ownerUsername, repoName, targetBranch, sourceBranch) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
    ...
    async forkBareRepo(sourceOwnerUsername, sourceRepoName, forkerUsername, newRepoName) {
        ...
        if (!fs.existsSync(sourceRepoPath)) {
            throw new NotFoundError_1.NotFoundError('Source repository disk contents not found');
        }
        //copy the entire bare reository contents
        fs.cpSync(sourceRepoPath, newRepoPath, { recursive: true });
    }
    async commitMultipleFiles(ownerUsername, repoName, branch, message, files, authorName, authorEmail) {
        await this.ensureRepoExists(ownerUsername, repoName);
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const indexFile = path.join(repoPath, `index-${Date.now()}.tmp`);
        const git = (0, simple_git_1.default)(repoPath);
        try {
            ...
            const commitHash = (await git
                ...
                .raw(['commit-tree', treeHash, ...parentAgrs, '-m', message])).trim();
    ...
    async isAhead(ownerUsername, repoName, base, head) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
            ...
            Logger_1.logger.error(`Error checking if ${head} is ahead of ${base}:`, error);
            return false;
        }
    }
    async deleteFile(ownerUsername, repoName, branch, message, filePath, authorName, authorEmail) {
        ...
        try {
            ...
            const parentHash = (await git.raw('rev-parse', branch)).trim();
            const commitHash = (await gitWithIndex.raw(['commit-tree', newTreeHash, '-p', parentHash, '-m', message])).trim();
            await git.raw(['update-ref', `refs/heads/${branch}`, commitHash]);
        }
        finally {
            if (fs.existsSync(indexFile))
    ...
    async getConflictDetails(ownerUsername, repoName, sourceBranch, targetBranch) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
            const mergeTreeResult = await git.raw(['merge-tree', targetBranch, sourceBranch]);
            ...
            for (const filePath of conflictPaths) {
                ...
                const conflictContent = this.buildConflictContent(oursContent, theirsContent, targetBranch, sourceBranch);
                conflictFiles.push({
                    path: filePath,
                    oursContent,
                    theirsContent,
    ...
    async resolveConflictsAndMerge(ownerUsername, repoName, sourceBranch, targetBranch, resolvedFiles, authorName, authorEmail) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        const indexFile = path.join(repoPath, `index-merge-${Date.now()}.tmp`);
        try {
            const gitWithIndex = (0, simple_git_1.default)(repoPath).env({
                ...process.env,
                GIT_INDEX_FILE: indexFile,
            });
            ...
            for (const file of resolvedFiles) {
                ...
                const tempFile = path.join(repoPath, `resolved-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
            ...
            const commitHash = (await git
                ...
                .raw(['commit-tree', treeHash, '-p', targetHash, '-p', sourceHash, '-m', commitMessage])).trim();
    ...
    async archiveRepo(ownerUsername, repoName, branch = 'main') {
        ...
        return new Promise((resolve, reject) => {
            ...
            const gitArchive = (0, child_process_1.spawn)('git', ['archive', '--format=zip', branch], {
                cwd: repoPath,
            });
            gitArchive.stderr.on('data', (data) => {
                Logger_1.logger.error(`Git archive error: ${data.toString()}`);
            });
            ...
            gitArchive.on('close', (code) => {
                if (code !== 0) {
                    ...
                    Logger_1.logger.error(`git archive exited with code ${code}`);
                }
            });
            // We resolve with the stdout stream immediately so the controller can pipe it
            resolve(gitArchive.stdout);
        });
    }
    async renameBranch(ownerUsername, repoName, oldBranchName, newBranchName) {
        const repoPath = this.getRepoPath(ownerUsername, repoName);
        const git = (0, simple_git_1.default)(repoPath);
        try {
            ...
            throw new Error(`Failed to rename branch: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
        }
    }
};
