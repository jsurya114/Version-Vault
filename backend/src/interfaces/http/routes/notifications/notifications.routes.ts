...
router.get('/', authMiddleware, (req, res, next) => controller.list(req as AuthRequest, res, next));
router.get('/unread-count', authMiddleware, (req, res, next) =>
  controller.unreadCount(req as AuthRequest, res, next),
);
router.patch('/:id/read', authMiddleware, (req, res, next) => controller.markRead(req, res, next));
